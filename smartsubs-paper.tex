...
Chinese-English subtitles. Learners enjoyed viewing videos
with Smart Subtitles just as much as with dual subtitles.
Learners' video comprehension levels were the same between tools,
both self-assessed and
...
to listen carefully to the word, pause the video, and look the word up in a dictionary. This is a time-consuming process which detracts from the enjoyability of watching the content. Alternatively, the learner could simply watch a version that is dubbed, or a version with subtitles in their native language to enjoy the content. However, they might not learn the foreign language effectively this way.
...
Videos in foreign languages have been adapted for foreign viewers and languages learners in many ways. These are summarized in Figure~\ref{fig:figure1} and are described in more detail below.
...
\caption{Mockups showing ways that a Chinese video can be presented
to English-speaking viewers and language learners.
Note that GliFlix does not actually support Chinese, and
...
listening comprehension ability \cite{danan2004captioning}. However, transcripts can harm comprehension, unless they are used by advanced
learners with good reading ability \cite{bianchi2008captions}.
...
However, in addition to a subtitle displaying the viewer's native language, they also display a caption in the original language as well. This way, a learner can both read the written representation and hear the spoken representation of the dialog in the foreign language, and will still have a translation
...
We developed a video viewing tool, Smart Subtitles, which displays subtitles to language learners to enhance the learning experience. It does so by providing support for dialog-level navigation features and vocabulary-learning features, which are shown in Figure~\ref{fig:figure2}. Smart Subtitles can be automatically generated for any video, provided that a caption is available.
...
\subsection{Design Goals of Smart Subtitles}

We designed Smart Subtitles based on discussions
with students learning Chinese about their experiences watching
Chinese videos.
We interviewed 5 students who had studied Chinese for at least
one year, who regularly watched Chinese-language dramas and movies
for their own enjoyment.
Students were asked to describe the types of aids they used when watching videos, and
any activities they did while watching videos to help them learn,
and what they tended to learn during the process of watching.

Students reported that they watched Chinese videos with English subtitles,
either by visiting online video streaming sites such as Youtube or Youku on their laptops, or by playing DVDs.
None viewed videos with no subtitles or just Chinese captions,
because the videos they watched used
complex language that they could not understand without subtitles.

Two students had previously viewed material that was presented
in the dual subtitles format. Both reported that they read the English
subtitle first, because their top priority was to understand the
video. One reported she did not bother reading the Chinese subtitle,
because there was not enough time after reading the English subtitle,
and she did not recognize Chinese characters well.
The other student
said that he would sometimes repeat the dialog to read the Chinese sutbtitle,
as he felt this helped him learn.

Students reported that they did not usually
pause subtitled videos when watching them for enjoyment.
However, one reported that he would repeat
lines that seemed like useful expressions.
Another student reported he occasionally
used an online dictionary to look up words in videos.
However, he did not usually do so while playing the video,
because switching windows to look up words was distracting.
Instead, he would just take note of words he wanted to learn while
watching the video, and look them up later.
Because he found it difficult to look up words in Chinese due to mishearing them, he usually just looked up English words from the English subtitle.

Students reported that watching videos was helpful for
refreshing their memory of material they had learned,
and combining words they already knew into new expressions.
However, with the exception of the student who jotted down words
and looked them up later, students reported they had only learned
a few new words from watching subtitled videos.

This led to our design goals for the Smart Subtitles interface:

\begin{enumerate}[noitemsep]
\item Should encourage users to try understanding the foreign-language dialog, before falling back on a translation.
\item Should provide romanizations, so it can be used by
people with limited ability to read the foreign script.
\item Should allow users to easily navigate through lines of dialog, so users can review and repeat them.
\item Should allow users to easily look up words from the dialog without pausing or leaving the video player, or entering words themselves.
\item Should not interfere with the users' ability to understand and enjoy the video.
\end{enumerate}

\subsection{Navigation Features}

We developed the navigation features of our interface based on foreign language learners' video viewing patterns. In our interviews, learners reported that they often reverse-seeked to the beginning of the current line of dialog to review the portion that had just been said. Therefore, we aimed to make this process as seamless as possible. In our interface, clicking on a section of the dialog will seek the video to the start of that dialog.
...
Sometimes, word-level translations are not enough for the learner to comprehend the current line of dialog. To address these cases, we include a button that shows learners a translation for the currently displayed line of dialog when pressed.
...
a system to extract subtitles and captions from videos, and an interactive interface to display them to learners.
...
\caption{Smart Subtitles uses subtitles in the WebVTT format
by default, but it can also extract subtitles from various other sources.}
\label{fig:figure3}
\end{figure}
...
we wrote an interface where the user
views the video, and presses the down button whenever a new
...
Overlayed-bitmap subtitles are pre-rendered versions of the text which are overlayed onto the video when playing. They consist of an index mapping time-ranges to the bitmap image which should be overlayed on top of the video at that time. This is the standard subtitle format used in DVDs, where is it called VobSub.
